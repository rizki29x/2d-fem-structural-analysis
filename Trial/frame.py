import numpy as np
import math
np.set_printoptions(5, suppress=True)

dof = []
noe = int (input("Input number of element(s) : "))
dof.append(noe)
non = int (input("Input number of nodal(s) : "))
dof.append(non)
print (dof)

cox = []
coy = []
for i in range(non):
    x = float (input("Input x coordinate value of node " +str(i+1)+ " : "))
    cox.append(x)
    y = float (input("Input y coordinate value of node " +str(i+1)+ " : "))
    coy.append(y)
print (cox, coy)

E = []
A = []
I = []
question1 = str (input("Do all element(s) have same material properties? (Y/N) : "))
if question1 in ["Y","y"]:
    elast = float (input("Input E value of all element(s) : "))
    cs_area = float (input("Input A value of all element(s) : "))
    inert = float (input("Input I value of all element(s) : "))
    for i in range(noe):
        E.append(elast)
        A.append(cs_area)
        I.append(inert)
elif question1 in ["n","n"]:
    for i in range(noe):
        elast = float (input("Input E value of element " +str(i+1)+ " : "))
        E.append(elast)
        cs_area = float (input("Input A value of all element " +str(i+1)+ " : "))
        A.append(cs_area)
        inert = float (input("Input I value of all element " +str(i+1)+ " : "))
        I.append(inert)
print (E, A, I)
    
initial = []
final = []
length = []
sinels = []
cosels = []
cons1 = []
cons2 = []
cons3 = []
for i in range(noe):
    start = int (input("Input initial node for element " +str(i+1)+ " : "))
    initial.append(start)
    end = int (input("Input end node for element " +str(i+1)+ " : "))
    final.append(end)
    
    x1 = cox [start-1]
    x2 = cox [end-1]
    y1 = coy [start-1]
    y2 = coy [end-1]

    L = math.sqrt((x2-x1)**2+(y2-y1)**2)
    length.append(L)
    sinel = (y2-y1)/L
    sinels.append(sinel)
    cosel = (x2-x1)/L
    cosels.append(cosel)
    con1 = (12*I[i])/(L**2)
    cons1.append(con1)
    con2 = (6*I[i])/L
    cons2.append(con2)
    con3 = E[i]/L
    cons3.append(con3)
print (initial, final, length, sinels, cosels, cons1, cons2, cons3)
print (I, A)

print (I)
node_sup = []
type_sup = []
nos = int (input("Input number of support : "))
tos = ('F = Fixed','P = Pinned','R = Roller')
for i in range(nos):
  ns_pos = int (input('Select node position of support '+str(i+1)+' : '))
  node_sup.append(ns_pos)
  print (tos)
  stype = str (input('Select type of support '+str(i+1)+' : '))      
  type_sup.append(stype)
print (node_sup, type_sup)

load_matrix = np.zeros((non*3,1))
print (load_matrix)
node_load = []
nol = int (input('Input number of load : '))
tol = ('1 = Horizontal load','2 = Vertical load','3 = Moment')
for i in range(nol):
    nl_pos = int(input('Select node position of load '+str(i+1)+' : '))
    node_load.append(nl_pos)
    print (tol)
    loadtype = str (input('Select type of load '+str(i+1)+' : '))
    if loadtype in ["1"]:
        magnitude = float(input('Input the magnitude of horizontal load '+str(i+1)+' in force unit : '))
        print ('Magnitude of horizontal load '+str(i+1)+' is: ')
        print (magnitude)
        load_matrix[nl_pos*3-3] = magnitude
    elif loadtype in ['2']:
        magnitude = float(input('Input the magnitude of vertical load '+str(i+1)+' in force unit : '))
        print ('Magnitude of vertical load '+str(i+1)+' is: ')
        print (magnitude)
        load_matrix[nl_pos*3-2] = magnitude
    elif loadtype in ['3']:
        magnitude = float(input('Input the magnitude of moment '+str(i+1)+' in  moment unit : '))
        print ('Magnitude of moment '+str(i+1)+' is: ')
        print (magnitude)
        load_matrix[nl_pos*3-1] = magnitude
print (load_matrix)
print ("where : ")
lmv = np.zeros((non*3,1))
lmv = np.asarray(lmv, dtype= str)
for i in range(non):
    lmv[0+i*3] = ('u'+str(i+1))
    lmv[1+i*3] = ('v'+str(i+1))
    lmv[2+i*3] = ('psi'+str(i+1))
print (lmv)

esms = []
gsms = []
print (A)
print (I)
for i in range(noe):
    S = sinels[i]
    C = cosels[i]
    SS = S**2
    CC = C**2
    CS = S*C
    f1 = cons1[i]
    f2 = cons2[i]
    f3 = cons3[i]
    esmx = np.array ([[(A[i]*CC)+(f1*SS), (A[i]-f1)*CS, -f2*S, -((A[i]*CC)+(f1*SS)), -((A[i]-f1)*CS), -f2*S],
                          [(A[i]-f1)*CS, (A[i]*SS)+(f1*CC), f2*C, -((A[i]-f1)*CS), -((A[i]*SS)+(f1*CC)), f2*C],
                          [-f2*S, f2*C, 4*I[i], f2*S, -(f2*C), 2*I[i]],
                          [-((A[i]*CC)+(f1*SS)), -((A[i]-f1)*CS), f2*S, (A[i]*CC)+(f1*SS), (A[i]-f1)*CS, f2*S],
                          [-((A[i]-f1)*CS), -((A[i]*SS)+(f1*CC)), -(f2*C), (A[i]-f1)*CS, (A[i]*SS)+(f1*CC), -(f2*C)],
                          [-f2*S, f2*C, 2*I[i], f2*S, -(f2*C), 4*I[i]]])
    esm = f3 * esmx
    esms.append(esm)
    print ('Element stiffness matrix of element '+str(i+1)+' : ')
    print (np.around(esm, 3))
    pos1 = initial[i]*3
    pos2 = final[i]*3
    position = [pos1-2, pos1-1, pos1, pos2-2, pos2-1, pos2]
    gsm = np.zeros(((non*3),(non*3)))
    for j in range(6):
        for k in range(6):
            p1 = position[j]-1
            p2 = position[k]-1
            gsm[p1,p2] = esm[j,k]
    gsms.append(gsm)
    print ('Global stiffness matrix of element '+str(i+1)+' : ')
    print (np.around(gsm, 3))
    
tgsm = sum(gsms)
print ('Total global stiffness matrix is :')
print (np.around(tgsm, 3))

tsms = tgsm
lm1 = load_matrix
lmv1 = lmv
dimf = np.ones(((non*3),1))
for i in range(nos-1, -1, -1):
    if type_sup[i] in ['f','F']:
        tsms = np.delete(tsms, node_sup[i]*3-1, 0)
        tsms = np.delete(tsms, node_sup[i]*3-1, 1)
        lm1 = np.delete(lm1, node_sup[i]*3-1, 0)
        lmv1 = np.delete(lmv1, node_sup[i]*3-1, 0)
        tsms = np.delete(tsms, node_sup[i]*3-2, 0)
        tsms = np.delete(tsms, node_sup[i]*3-2, 1)
        lm1 = np.delete(lm1, node_sup[i]*3-2, 0)
        lmv1 = np.delete(lmv1, node_sup[i]*3-2, 0)
        tsms = np.delete(tsms, node_sup[i]*3-3, 0)
        tsms = np.delete(tsms, node_sup[i]*3-3, 1)
        lm1 = np.delete(lm1, node_sup[i]*3-3, 0)
        lmv1 = np.delete(lmv1, node_sup[i]*3-3, 0)
        dimf[node_sup[i]*3-1] = 0
        dimf[node_sup[i]*3-2] = 0
        dimf[node_sup[i]*3-3] = 0
    elif type_sup[i] in ['P','p']:
        tsms = np.delete(tsms, node_sup[i]*3-2, 0)
        tsms = np.delete(tsms, node_sup[i]*3-2, 1)
        lm1 = np.delete(lm1, node_sup[i]*3-2, 0)
        lmv1 = np.delete(lmv1, node_sup[i]*3-2, 0)
        tsms = np.delete(tsms, node_sup[i]*3-3, 0)
        tsms = np.delete(tsms, node_sup[i]*3-3, 1)
        lm1 = np.delete(lm1, node_sup[i]*3-3, 0)
        lmv1 = np.delete(lmv1, node_sup[i]*3-3, 0)
        dimf[node_sup[i]*3-2] = 0
        dimf[node_sup[i]*3-3] = 0
    elif type_sup[i] in ['R','r']:
        tsms = np.delete(tsms, node_sup[i]*3-2, 0)
        tsms = np.delete(tsms, node_sup[i]*3-2, 1)
        lm1 = np.delete(lm1, node_sup[i]*3-2, 0)
        lmv1 = np.delete(lmv1, node_sup[i]*3-2, 0)
        dimf[node_sup[i]*3-2] = 0

print ('Stiffness matrix (simplify) is : ')
print (np.around(tsms,3))
print ('Load matrix (simplify) is : ')
print (lm1)
print (lmv1)

tsms_invers = np.linalg.inv(np.around(tsms,3))
dism = np.matmul(tsms_invers, lm1)
print ('Displacement Matrix is : ')
print (dism)
print ('where :')
print (lmv1)
print (len(dism))
for i in range(len(dism)):
  for j in range(non*3):
    if dimf[j] == 1:
      dimf[j] = dism[i]
      break
    else:
      dimf[j] = dimf[j]
print ('Total displacement matrix is :')
print (dimf)